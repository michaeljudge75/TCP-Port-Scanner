...
fn test_scan_range_length_is_correct(){
    ...
    let results = scan_range("127.0.0.1", start, end, 50, 10);
...
fn test_scan_range_perserves_order(){
    let results = scan_range("127.0.0.1", 5, 8, 50, 10);
...
fn test_scan_range_rate_limit_delay(){
    ...
    let t0 = Instant::now();
    let _ = scan_range("127.0.0.1", start_port, end_port, 50, rate_limit);
...
fn test_scan_range_propgates_errors(){
    let results = scan_range("invalid.invalid.invalid", 1, 3, 50, 10);
